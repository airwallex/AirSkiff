...
public class FlinkBatchCompiler extends AbstractFlinkCompiler {
  ...
  protected <K, T> KeyedStream<Tuple2<Long, Pair<K, T>>, K> compileOrderedSum(
    ...
    return new KeyedStream<>(ks.window(WINDOW)
      .process(new ProcessWindowFunction<Tuple2<Long, Pair<K, T>>, Tuple2<Long, Pair<K, T>>, K, TimeWindow>() {
        @Override
        public void process(
          K k,
          Context context,
          Iterable<Tuple2<Long, Pair<K, T>>> iterable,
          Collector<Tuple2<Long, Pair<K, T>>> collector
        ) throws Exception {
        ...
        new PairTypeInfo<>(Utils.typeInfo(stream.keyClass()), Utils.typeInfo(StreamUtils.kStreamClass(stream)))
      )), t -> t.f1.l);
  }
